...
public class SoundCardHelper {
	...
	public static javax.sound.sampled.AudioFormat getAudioFormat() {
		...
		int sampleSizeInBits = 8; // TODO : (int) configuration.getAudioFormat().getSampleSizeInBits(); only 8
									// bits supported for now
		int channels = (int) configuration.getAudioFormat().getChannels();
		boolean signed = true;// TODO : configuration.getAudioFormat().getSigned(); only signed supported for
								// now
		boolean bigEndian = true; // TODO : configuration.getAudioFormat().getBigEndian(); only bigEndian
									// supported for now
	...
	public static double removeNoise(byte sample) {
		/*
		 * short sample = (short) (amplitude & 0xFF); int sample2=sample;
		 */
		byte sample2;
		if (sample < 0)
			sample2 = (byte) -sample;
		...
		if (sample2 <= configuration.getNoiseLevel())
			...
			return (double) (sample2 - configuration.getNoiseLevel())
					* ((double) 128 / (128 - configuration.getNoiseLevel()));
		}
	}
...
//				testAmplitude(amplitude);
			...
			amplitudes[channel] = Math.max(0,
					Math.min(128, integralFactor * Math.sqrt(((double) channels * sumOfSquares) / (double) bytesRead))); // -
																															// 80));
		}
		return amplitudes;
	}
	...
	public static TargetDataLine findTargetDataLine(String mixerName, Line.Info selectedLineInfo) {
		// Find the desired mixer
		TargetDataLine targetDataLine = null;
		Mixer.Info[] mixerInfos = AudioSystem.getMixerInfo();
		Mixer selectedMixer = null;

		//	logger.debug("Looking for mixer " + mixerName);

		for (Mixer.Info mixerInfo : mixerInfos) {
			// logger.info("Checking mixer :" + mixerInfo.getName());

			if (mixerInfo.getName().equals(mixerName)) {
				selectedMixer = AudioSystem.getMixer(mixerInfo);
				//logger.info("Mixer found:" + mixerInfo);
				break;
			}
		}
		...
		if (selectedMixer != null) {
			...
			//logger.debug("Looking for line info matching " + selectedLineInfo);

			// Find the desired port in the selected mixer
			Line.Info[] targetLineInfos = selectedMixer.getTargetLineInfo();
			for (Line.Info targetLineInfo : targetLineInfos) {
				//logger.info("Checking target line :" + targetLineInfo);
				if (targetLineInfo instanceof DataLine.Info) {
					try {
						targetDataLine = (TargetDataLine) selectedMixer.getLine(selectedLineInfo);
						//logger.info("targetDataLine found:" + targetDataLine);
						break;
					} catch (LineUnavailableException e) {
						//logger.info("LineUnavailableException " + selectedLineInfo, e);
					}
				}
			}

		}
		//logger.error("could not get line from mixer");
